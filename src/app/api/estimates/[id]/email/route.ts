...
import { loadCompanySettings, sendCompanyMail, smtpConfigured, smtpErrorMessage, resolveMailIdentities } from "@/lib/mail";
...
export async function POST(
  ...
  const replyTo = identities.replyTo;
  const cc = String(body.cc !== undefined ? body.cc : replyTo || "")
    .split(/[,;]/)
  ...
  const text =
    ...
    estimateMailMessage(estimate.number, estimate.vehicle, settings.name, replyTo);
